...
def init_checkers(session, platforms=None, proxy_list=None):
    checkers = {}
    for platform in platforms if platforms is not None else list(Platforms):
    ...
    try:
        ...
        if is_email and issubclass(platform.value, EmailQueryable):
            ...
            if response is None:
                raise QueryError("Error retrieving result")
            return response
    # KeyError/IndexError/ValueError cover a response whose shape no longer matches what the
    # platform's parsing expects, including a body that is not the JSON its headers promised
    except (
        aiohttp.ClientError,
...
async def execute_queries(queries, platforms=None, proxy_list=None):
...
def sync_execute_queries(queries, platforms=None, proxy_list=None):
