...
def init_parser():
    ...
    parser.add_argument(
        ...
        metavar="query",
        nargs="*",
        help="one or more usernames/email addresses to query (email addresses are automatically queried if they match the format)",
    )
    parser.add_argument(
        "--platforms",
        "-p",
    ...
    parser.add_argument(
        ...
        "-c",
        action="store_true",
        help="cache tokens for platforms requiring more than one HTTP request (Instagram & Lastfm), reducing total number of requests sent",
    )
    parser.add_argument(
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if args.cache_tokens:
            ...
            # A platform that will not hand out a token is reported per query rather than
            # aborting the whole run
            await asyncio.gather(
                *(init_prerequest(platform, checkers) for platform in platforms),
